...
> **Status: pruned 2026-07-10.** Originally written 2026-07-06 from a design review of the three
> repos that expose kzen-auto to end users: **kzen-shell** (desktop composition root: process
> manager + single-port reverse proxy), **kzen-launcher** (project selector UI), and
> **kzen-project** (the blank product template a spawned project runs). Much of the original plan
> landed in the 0.29.x release train — the proxy rework (shared Ktor client, faithful headers,
> all-methods route, 503/502 error semantics), bounded start + the ConcurrentHashMap
> STARTING/RUNNING/STOPPING/FAILED registry state machine, adaptive UI status polling, the
> config/distribution pipeline (`dist` tasks, properties resolution, atomic artifact acquisition —
> see `../docs/RELEASING.md` and each sibling's AGENTS.md), the zip-slip guard, and the
> delete-confirm dialog. This revision keeps only what remains, with the practical benefit of each
> phase stated up front.
>
> Executor: **Opus 4.8 xhigh, one phase per session.** Each phase is self-contained: benefit,
...
- **Remote / multi-machine serving, HTTPS, auth tokens**: the loopback-only contract stays; phase
  1 hardens *within* it (cross-site attacks arriving through the local browser). Remote serving
  would be a new front door, not a loosening of this one.
...
**Benefit:** any web page the user visits can currently fire cross-site GETs at
`http://localhost:8080/…` that recursively delete a project's files (`ProjectRepo.delete` →
`deleteRecursively()`), spawn any jar already on disk (`/shell/project/start?location=<path>` —
the browser supplies the path), and drive every proxied kzen-auto command endpoint (file writes,
Script execution = arbitrary code execution); DNS rebinding extends the same reach to remote
attackers. Separately, both DownloadServices install a JVM-global trust-all
`SSLSocketFactory` + hostname verifier while downloading *executable jars* — a network MITM can
substitute code. This phase closes all of it at the single ingress, protecting every child
uniformly.
...
- **Fetch-metadata + Host gate at the shell**, as a Ktor plugin/interceptor applied before
  ...
  `same-site` / `none` (kills cross-site requests from pages; every modern browser sends it;
  requests without the header — curl, same-machine tools — pass, which is the accepted residual
  risk on a single-user desktop). Top-level navigations (`Sec-Fetch-Mode: navigate` to GET) stay
  allowed so external links into the app work — mutating shell endpoints and proxied non-GETs are
  never navigations. The same gate is duplicated (small, dependency-free) in the launcher for
  standalone dev runs.
- **No auth token, no CORS machinery** — fetch-metadata is sufficient for the loopback threat
  model and adds zero UX friction. A token only returns if remote serving ever does.
- **`start` stops trusting the browser for executable paths as defense-in-depth** (the gate
  already blocks cross-site): the shell requires `location` to resolve to an existing file named
  exactly `main.jar`; normalize and log. Keep it proportionate — the gate is the real fix.
- **Delete `trustBadCertificate()` from both repos** (shell DownloadService.kt:23-39, launcher
  DownloadService.kt:21-37). GitHub releases and any sane artifact host have valid chains; a
  corporate-MITM user can add `-Djavax.net.ssl.trustStore` themselves — document in README.
- **Server-side project-name validation** in `RestHandler.createProject`/`renameProject`: single
  path segment (no `/` `\` `..`), not empty, ≤128 chars, not a reserved routing name (`main`,
  `shell`), no Windows-reserved device names (CON, NUL, …). Reject → 400 with message (the UI
  already surfaces ErrorBus messages). Today `..\..\x` escapes the project home
  (`projectHome.resolve(name)`, ProjectCreator.kt; rename via `resolveSibling`), and `main` /
  `shell` collide with the routing contract.
- Mutating endpoints **stay GET** — the gate is the protection; migrating the wire to POST is
  churn with no additional security.
...
4. Both DownloadServices: delete trust-all + the call sites (KzenShellContext.kt:44,
   KzenLauncherMain.kt:161-164); README note.
5. Launcher: name-validation helper in kzen-launcher-jvm used by create + rename.
6. Regression: `curl -H "Sec-Fetch-Site: cross-site" http://localhost:8080/shell/project/stop?name=x`
...
stop → rename → delete); a hand-written `<img src>` CSRF page no longer triggers stop/delete;
`create?name=..%2Fpwn` → 400; archetype download over https works with real certificate
validation.
...
groundwork (bounded start, registry state machine, adaptive polling) makes this phase much
smaller than originally scoped — it is the missing second half of failure containment.
...
- **First tests for kzen-shell** (currently zero): JUnit coverage of the
...
**Premise change since the original plan:** kzen-project's 2026-06-21 KSP migration left zero
`@Reflect` classes, so KSP emits no modules and the mains correctly register nothing — empty
modules must NOT be re-added on their own. The extension point is still broken for downstream
users, though: adding an `@Reflect` class to the template (its entire purpose) generates a module
that nothing registers → runtime instantiation failure with no hint why. The fix is a package
deal: a sample `@Reflect` object (which makes the generated modules non-empty) plus the
registration calls plus a test.
...
7. **Docs-to-truth sweep**: bring shell/launcher AGENTS.md in line with whatever phases 1–4
   changed (gate, status shape, `--project.home`, upgrade action).
...
| Phase | Size | Risk |
|---|---|---|
| 1 — trust hardening | M | Low-Medium (gate false-positives) |
...
- **Reserved names**: `main` (launcher alias) and `shell` (first-class endpoints) are unreachable
  as project names; phase 1 rejects them at create/rename.
