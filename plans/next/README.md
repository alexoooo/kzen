...
| `CX_context-generalization.md` | CX1–CX8 — context generalization arc | context-improvements | 33–40 | Elaborated 2026-08-02 with anchors re-verified (four factual corrections to the CX doc recorded in its §0, plus one resolved inconsistency: **CX7 is cross-repo** — its `Execution.host` extension is kzen-lib work despite the §5 prose). CX1 is the independent pull-forward; CX2→CX3 are the kzen-lib substrate; CX4+CX6 sweep the same notation files, sequence adjacently; CX8 stays a design gate |
