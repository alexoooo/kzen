...
| `nested-frame-move-to.md` | XC-N — nested-frame move-to (Set Next Statement inside a sub-Script) | — (standalone: design + elaboration in one document) | 30–31 | ✅ **LANDED 2026-07-30** — both sessions; see its § As-built for the deviations (two additions to the kzen-lib surface the plan did not foresee, and the recursion test case dropped as unassertable). **Filed as a DEFECT, not an extension** (§10.2): no spec asserted the root-frame restriction, and `Execution.moveTarget`'s KDoc anticipated the opposite. Shipped: **XC-N1** (straight-line / `If`-nested hops). Still open: **XC-N2** (loop-hosted hops), which needs the parked loop-body extension first and is now a *real* rejection rather than a silent no-op. §4.1's untested prediction was confirmed by observation before any design work. ⚠️ Standalone ⇒ archive, don't delete |
