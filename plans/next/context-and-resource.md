...
> where they change a decision rather than add detail. Blast radius confirmed contained: kzen-project,
...
- **[review] Four tests that pin the corrections specifically** — each one is a case an earlier draft of this plan got wrong, so none of them is optional:
...
|---|---|---|
| **A** | Phases 1 + 2, one commit — including the kzen-lib doc surfaces (logic-spec §6 + §2 cross-ref, `ResourceClosePolicy` KDoc), since they ship with the enum | `cd ../kzen-lib && ./gradlew build` green (rewritten `RunEngineTest` block incl. the undeclared-key-to-Self test); `publishToMavenLocal` for all 4 subprojects; `cd ../kzen-auto && ./gradlew build` green; `ScriptExtensibilityTest` green against migrated fixtures (§2 a/d); the closer-tolerance and `contextValueOrNull`-off-`provides` tests from §6 pass |
| **B** | Phase 3 + the §2 (b)/(c) notation sweep | `ScriptContextValidationTest` green incl. both unslotted-provide fixtures and the `releases:` case; all 17 self-test documents carry their declarations (15 `context.requires`, 2 providers) and the 4 owners declare `context.slots`; opening FizzBuzz in the editor shows **zero** unexpected warnings (the `main\Script.yaml` harness warning is expected and documented) |
