...
**Gotcha — Java 26 toolchain vs PATH `java`.** Gradle compiles with the JDK 26 toolchain (auto-detected from `~/.jdks`; `org.gradle.java.installations.auto-download=false`, so JDK 26 must be installed) regardless of your PATH, but `java -jar <built>.jar` uses whatever `java` is first on `PATH` (often Java 8 on this machine, which fails with `UnsupportedClassVersionError: class file version 70.0`). Run with an explicit Java 26: `& "C:/Users/ostro/.jdks/temurin-26.0.1/bin/java" -jar build/libs/...jar` (the user's IntelliJ-managed JDK pool lives at `~/.jdks/`). Note the daemon itself still runs on Java 25 — Gradle 9.6.1 does not yet support Java 26 as the launcher JVM, only as a toolchain target.
...
- Kotlin `2.4.0`, KSP (`com.google.devtools.ksp`) `2.3.9`, Ktor `3.5.1`, JVM toolchain & target `26`
- Jackson 3.x **YAML only** (`tools.jackson.core:jackson-databind:3.2.0` + `tools.jackson.dataformat:jackson-dataformat-yaml:3.2.0`) — used **solely by kzen-launcher's `ProjectRepo`** for the `kzen-projects.yaml` registry; note the `tools.jackson` group, not the legacy `com.fasterxml.jackson` one. SER5 removed all JSON Jackson (`jackson-module-kotlin` + `ktor-serialization-jackson`) from kzen-auto and kzen-shell — both are now Jackson-free, with kotlinx.serialization the single JSON codec
...
- kotlinx-serialization-json `1.9.0` (`kotlin("plugin.serialization")` version = `kotlinVersion`) — applied in the `-common` modules of `kzen-launcher` (SER1), `kzen-lib`, and `kzen-auto` (SER2), plus `kzen-auto-jvm`/`-js` (SER3) and the single-module `kzen-shell` (SER5); the single structured-wire JSON codec the SER track converged on — every server now serves JSON via kotlinx (Ktor `json()` ContentNegotiation), Jackson survives only for launcher YAML
- Kotlin/JS frontends use `org.jetbrains.kotlin-wrappers:kotlin-wrappers-catalog` (declared in `settings.gradle.kts` of `kzen-auto` and `kzen-launcher` only — `kzen-project` and `kzen-lib` pull wrappers transitively; exposed as the `kotlinWrappers` version catalog). **Both kzen-launcher and kzen-auto are at `2026.7.1`** (bumped to 2026.7.1 on 2026-07-07 alongside the JVM 26 / Gradle 9.6.1 update; previously `2026.6.5` from 2026-06-21 alongside the Kotlin 2.4.0 toolchain update, and `2026.5.3` migrated 2026-05-11 — kzen-launcher migrated first and supplied the template kzen-auto applied same-day). The migration template lives in each sibling's `wrap/React.kt`: re-implemented `RPureComponent` (extends `Component` + `shouldComponentUpdate` shallow-compares props/state — replaces removed `react.PureComponent`), a `KClass<out Component<P, *>>.react: ComponentType<P>` extension (replaces the removed `kotlin-react-legacy` member; cast via `js.reflect.unsafeCast`), and a `createRef<T>()` top-level bridge (replaces removed `react.createRef` — `useRef` is a hook and class components can't call hooks). Copy verbatim when migrating future JS siblings; only the package path differs. Surface-level breakage in 2026.x consumers also includes: `key = "stringExpr"` must wrap in `react.Key(...)`; `ChangeEvent<C, T>` two type args mandatory and `event.target` returns the second arg while `event.currentTarget` returns the first (use `.currentTarget.checked` for `<input>` checkbox handlers); class components that previously got `setState { ... }` via the now-removed `PureComponent.setState` extension need an explicit `import tech.kzen.auto.client.wrap.setState`. **`useCommonJs()` is load-bearing across the kzen ecosystem** — `useEsModules()` was attempted in kzen-launcher and reverted; `@mui/icons-material` is CommonJS-packaged and every icon errors with `'default' (imported as 'createSvgIcon') was not found` under ESM resolution. The block goes away once MUI is bumped to a version that ships ESM. The launcher's `muiIconsVersion` is pinned to **`9.2.0`** to match the `@mui/material` version the wrappers BOM resolves (NOT npm-latest — the 2026.7.1 BOM resolves `@mui/material@9.2.0`, up from `6.5.0` under 2026.6.5) — a mismatched major re-triggers the peer-dependency warning; re-check the resolved `@mui/material` in `kzen-launcher/kotlin-js-store/yarn.lock` on any wrappers bump. For kzen-auto's broader JS architecture, see [`../kzen-auto/docs/js-architecture.md`](../kzen-auto/docs/js-architecture.md).
...
When bumping Kotlin (even patch versions like `2.4.0` → `2.4.x`):
...
- `kzen-shell`, `kzen-lib`, `kzen-auto`, `kzen-project`, `kzen-launcher` all share a single version, a **coordinated release train** — bump them together (see [`docs/CODING_STANDARDS.md`](docs/CODING_STANDARDS.md)).
