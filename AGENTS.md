...
- Kotlin/JS frontends use `org.jetbrains.kotlin-wrappers:kotlin-wrappers-catalog` (declared in `settings.gradle.kts` of `kzen-auto` and `kzen-launcher` only — `kzen-project` and `kzen-lib` pull wrappers transitively; exposed as the `kotlinWrappers` version catalog). Versions diverge per sibling: **kzen-launcher is at `2026.5.3`** (bumped 2026-05-11), **kzen-auto is still pinned at `2025.12.11`**. The empirical ceiling on kzen-auto remains `2025.12.11`. Confirmed breakage at the 2026.x line: (a) wrappers `2026.2.20`+ removes `kotlin-react-legacy` — in kzen-auto only 1 file imports legacy types directly (`kzen-auto-js/.../client/wrap/select/reactSelectDsl.kt`); (b) wrappers `2026.2.11`+ changes the React `key` attribute to `Key?` (string-literal breakage in `OutputTableController.kt`, `GraphController.kt`) and `ChangeEvent<C>` requires two type args (breaks `AttributePathValueEditor.kt`, `graph/edit/AttributePathValueEditorOld.kt`); (c) `2026.x` also removes `react.PureComponent` and the `KClass<C>.react` extension that used to live in `kotlin-react-legacy`. The kzen-launcher upgrade handled (c) with two pieces of scaffolding in its own `wrap/React.kt`: a `KClass<out Component<P, *>>.react` bridge (casts `KClass.js` to `ComponentType` via `js.reflect.unsafeCast`), and a re-implemented `RPureComponent` (just `Component` + a `shouldComponentUpdate` that shallow-compares props/state, which is what `react.PureComponent` always was). Both idioms should carry over verbatim when kzen-auto migrates. For kzen-auto's broader pre-refactor picture, see [`../kzen-auto/docs/js-architecture.md`](../kzen-auto/docs/js-architecture.md).
