...
- Jackson 3.x (`tools.jackson.module:jackson-module-kotlin:3.2.0`, `tools.jackson.dataformat:jackson-dataformat-yaml:3.2.0`) — note the `tools.jackson` group, not the legacy `com.fasterxml.jackson` one
...
- kotlinx-serialization-json `1.9.0` (`kotlin("plugin.serialization")` version = `kotlinVersion`) — applied in the `-common` modules of `kzen-launcher` (SER1), `kzen-lib`, and `kzen-auto` (SER2); the single structured-wire JSON codec the SER track is converging on
