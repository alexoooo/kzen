...
- `../kzen-repo` (forked-artifact mirror, no Kotlin source), `../kzen-sample-plugin` (Maven sample plugin, no Gradle source) and `../kzen-sample-embed-spring` (Maven Spring Boot host embedding kzen-auto workspaces, plugin zero over the sample; no Gradle source) → [docs](../kzen-sample-embed-spring/AGENTS.md) are also `includeBuild`d but contain nothing to bump
...
**Working directory matters.** Gradle resolves *artifact substitutions* from the umbrella or from any included build alike, but *task addressing* does not: anything that actually compiles or tests must be invoked from the sibling's own directory. The wrapper is Gradle 9.6.1. Three distinct JVM roles: the **Gradle JVM** (`JAVA_HOME` / IDE Gradle JVM) must be ≥ 25 because buildSrc targets Java 25 bytecode — JDK 25 and 26 both work (verified with `--no-daemon help` on temurin-26.0.2); the **compile toolchain** is JDK 25 (`jvmToolchainVersion` in every `Dependencies.kt`, auto-detected from `~/.jdks`), so Java and Kotlin compile against the Java 25 API and emit class-file 69; the **supported runtime** is therefore Java 25 or newer — a foreign host on JDK 25 can load every artifact.
...
**Gotcha — Java 25 toolchain vs PATH `java`.** Gradle compiles with the JDK 25 toolchain (auto-detected from `~/.jdks`; `org.gradle.java.installations.auto-download=false`, so a JDK 25 must be installed) regardless of PATH, but `java -jar <built>.jar` uses whatever `java` is first on `PATH` (often Java 8 on this machine → `UnsupportedClassVersionError: class file version 69.0`). Run with an explicit Java 25 or newer: `& "C:/Users/ostro/.jdks/temurin-25.0.4.1/bin/java" -jar build/libs/...jar` — **check `~/.jdks` for the actual patch version before copying that path**, it moves with each JDK update and a stale one fails as a bare `No such file or directory` (exit 127), not as a Java error. A Gradle JVM of ≤ 24 fails to resolve `:buildSrc` (its bytecode targets 25) — the symptom is the misleading `e: build.gradle.kts:2: Unresolved reference 'kotlinVersion'` at configuration time (buildSrc's constants can't load on the older JVM, so every constant the root script reads is "unresolved"); fix the JVM, not the script.
...
- Kotlin `2.4.0`, KSP (`com.google.devtools.ksp`) `2.3.9` (kzen-lib, kzen-auto, kzen-project), Ktor `3.5.1` (kzen-auto, kzen-launcher, kzen-shell), JVM toolchain & target `25` (runtime baseline Java 25+; the toolchain, not just the target, is 25 so no Java-26-only API can slip into the release train)
