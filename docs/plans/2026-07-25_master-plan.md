...
| **E** | `2026-07-25_extensibility-improvements.md` | Plugin = classloader (process-global `KzenAutoRuntime`), plugin UX, Custom power, plain-object data shape, object-graph paths, closeable streams and items; absorbs reflection R5/R6. **E1 ratified 2026-09-04; review 1 of the hosting analysis folded in** | E2–E4, E7–E9 |
...
| 10 | **E2** — plugin = classloader | One **process-global `KzenAutoRuntime`** (initialized once before any context; conflicting re-init fails fast) owning one `URLClassLoader` per `plugins/<name>/` jar set (`--plugin.root=`, deterministic order, duplicates are boot errors, per-scope failure isolation), app loader as plugin zero; per loader: `ServiceLoader` readers de-duplicated by declaring loader, `ReflectiveClassMirror(loader)` on `GlobalMirror` (lazy — no boot validation promise), bundled notation; `dynamicParentClassLoader()` returns the runtime's composite loader (zero call-site changes); `BlockingReaderCapability` + **cursor-driven** `SourceWorker` in `kzen-auto-plugin`; optional manifest with SPI-compatibility version; `plugins.yaml` / plugin `ReportDefiner` discovery retired. Verified pure-Java from a folder (sample plugin, unshaded) **and** as a dependency **and** Kotlin+KSP, plus a reusable plugin compatibility test kit. **L; needs the HS Java 25 baseline first** | E · E2 | — | ☐ |
...
| 61 | **E9** — closeable streams and items | **Added 2026-09-04 (user requirement via review 1 of the hosting analysis).** `AutoCloseable` honoured on every expression stream type (`Iterable` / `Iterator` / `Sequence` + new `java.util.stream.Stream`) — closed on completion / cancel / failure, detached and re-adopted across live-edit migration instead of re-evaluated; and on **individual elements** — a per-run ledger keyed by native identity with a reference count over channel hand-offs (closed when the last consumer moves past it), owner propagation to navigation children and to non-scalar Formula / Filter outputs, flush-on-send for owned elements, carried unclosed through `drainBuffered` / `preload`, all closed at teardown, post-close access a named error, and a stall diagnostic naming an accumulating Worker that holds owned items (no validation rule — inherent complexity, surfaced not prevented). This is the mechanism behind the in-process host's memory arena (a symbol-day batch consumes the arena on open, releases on close); kzen needs no admission concept. **M–L; independent of E2 and E7** | E · E9 | — | ☐ |
