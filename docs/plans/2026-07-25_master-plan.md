...
| 49 | **DS0** — data-layer package moves out of Report | **DS arc prep** (`docs/analysis/2026-08-20_job-data-source.md` §10). Mechanical package/import work: move `HeaderListing` / `HeaderLabel` / `HeaderLabelMap` to `common/data/schema/`, and move the flat-input and listing plumbing row 52 consumes out of Report-owned packages. Method bodies do not change. **Must precede rows 50 and 52** so the new abstractions do not cement Report ownership. **M** | — (analysis) | `next/DS0` | ☐ |
