...
|---|---|---|---|---|---|
| 1 | **J3a** — pluggable input formats | ~~`PluginReaderWorker` bridging the plugin SPI + `encoding` on native readers~~ **SUPERSEDED 2026-08-21 by DS2/DS3** (rows 50–51): the plugin-format read lives in `FileDataSource.rows`, `encoding` in `DataPart.encoding` | J · P3 steps 1,4 | `next/J3` | ⊘ |
| 2 | **J3b** — design-time services | ~~Column pre-scan route + `JobUpstreamSchema` fallback on `WorkerLane` + `SortSpecEditor` dropdown~~ **SUPERSEDED 2026-08-21 by DS6** (row 54): the pre-scan is the source object's detached `schema` action, the fallback is a provider list on `JobUpstreamSchema` | J · P3 steps 2,3 | `next/J3` | ⊘ |
...
| 50 | **DS1** — data-source value model + lowering | **Start of the DS arc (Job data sources), added 2026-08-21 on direct user request; rationale lives in `docs/analysis/2026-08-20_job-data-source.md` (+ `2026-08-21_extension-points.md`), which is the permanent record — the `next/DS*` files are deleted on landing and as-built notes go to the analysis doc §14.** `DataRole` / `DataRef` / `DataPart` / `DataUnit` / `DataManifest` / `DataSourceId` in kzen-auto-common with digest, `ExecutionValue` lowering, kotlinx wire form, `DataLocation` ⇄ `DataRef`, and the construction helpers a user-authored source needs. **`DataRef.source` is a MINTED DURABLE `DataSourceId`, not an `ObjectStableId`** — the mapper is in-memory and session-scoped, so a renamed source's persisted ref dies on restart (§3.3, C9). **S–M, common only, no consumers yet** | — (analysis) | `next/DS1` | ☐ |
| 51 | **DS1b** — expression type-visibility fix | **A standing bug the DS arc would have tripped over, found 2026-08-21b** (§5.5a, C2): `ExpressionReturnTypeInference.toTypeMetadata` erases any classifier outside an eleven-entry hardcoded `visibleBuiltins` and the one-entry `ObjectRegistry` down to `Any` — so a `DataUnit` on a lane would type as `Any` and the expression route's headline case would not compile. Replace the whitelist with a **public-and-nameable predicate** (no hardcoding, no `ObjectRegistry` round-trip for first-party code), and `isIterable` with **`isStreamType`** covering `Iterable \| Sequence \| Iterator` (O6). Independent of the arc and worth landing regardless. **S; may ride inside row 50** | — (analysis) | `next/DS1b` | ☐ |
| 52 | **DS2** — `DataSource` + `DataScope` + `FileDataSource` | The SPI (`units` / `items` / `schema` / `itemType` + constrain-once `DataItems: Sequence` whose non-null `flatHeader` means flat records), **non-suspend with a `DataScope`** carrying the quiescence-visible offload, named arguments and the Context borrow (C1, D1); the file source as a `@Reflect` notation object over the existing `FileListingAction` (split into a blocking core) / `FileFlatDataSource` / `ReportDefiner` path (**this is J3a's reader re-homed into the source object** — one unit per file, role `main`, named-capture group pattern → unit attributes, `missing: skip \| fail`); the `DataSources` document with graph-wide discovery on the `Contexts` pattern (D3); `DetachedAction` `resolve`; the Job `sources:` branch; the input plumbing moved out of Report (§10). **Spike O14 (nullable structural reference) first.** Third-party sources are dynamically loaded objects, not a source SPI. **M–L** | — (analysis) | `next/DS2` | ☐ |
| 53 | **DS3** — `ReadWorker` (source-generic declarative reader) | `source:` a **nullable structural reference** (`is: DataSource, nullable: true`) so the instance comes from the run's own graph, not `GraphInstanceCache` (D2); `emit: items \| units` (O11), `role:`, multi-role units flow whole, resolved manifest **carried** across migration (never re-resolved) with a positional cursor guarded on the source's definition digest (C7), `payloadFlow` from `itemType` + the pre-scan cache with no IO. Heterogeneous item schemas **fail loudly** — because `SummaryWorker` and `CsvWriterWorker` would otherwise lose data silently (C3, §5.2b); superset normalization lands in row 56. Removes `CsvReaderWorker` / `MultiFileReaderWorker` from the ribbon (archetypes stay). A/B vs both old readers. **M–L** | — (analysis) | `next/DS3` | ☐ |
| 54 | **DS4** — sources editing surface | Job "Data sources" section + `Data` ribbon group (insert nests under `sources` **by capability**, `drop(1)` so the abstract archetype does not offer itself), `DataSourceCard` rendering attributes through `AttributeEditorManager` with generic **Resolve** / **Columns** chrome over the detached action, `FileSelectionEditor` (= `MultiFileInputEditor` rewritten as `FileDataSource.files`' editor, with the `filter` field labelled for what it is — a contains-all-words match, **not** a glob, C6), `SelectDataSourceEditor` with **auto-bind when exactly one source** plus an in-card bound-source summary. **Zero file-specific code in the Job editor.** ⚠ An `editor:` key and its `@Reflect` registration must land together — a name miss renders `[Attribute editor not found]` with no input, it does **not** fall back (C5). **L, split 1–3 / 4–5; browser smoke is the deliverable — with row 53 this makes "count the lines of one file" three cards and no code** | — (analysis) | `next/DS4` | ☐ |
| 55 | **DS5** — `ExpandWorker` + objects in expression scope | The 1:N expression transform (O1: new archetype, dispatch lifted from `FormulaSourceWorker`), transform emit cadence (`Emitter.expandCadence`), `AutoCloseable`-aware + re-read-on-migrate (O7, the deliberate contrast to `ReadWorker`'s positional cursor), the Job's `sources/` objects as bare typed accessors (`JobControl.objects()/obj()`) **sourced from the run's `graphInstance`** (D2), `items(part)` / `unit.items(role)` helpers dispatching through `DataRef.source` (self-opening across a Logic boundary). Flat-vs-payload emission keys on the `flatHeader` **interface member**, so the worker stays domain-free. **This is what `groupBy` parity depends on** — unit attributes do not reach the item lane by themselves (C4). Independent of rows 53–54. **L** | — (analysis) | `next/DS5` | ☐ |
| 56 | **DS6** — design-time schema | `FileDataSource.schema` (declared, or a bounded header read), **cache key `(ref.id, format, encoding, size, mtime)`** in `ColumnListingAction` (fixes Report's stale-`columns.csv` bug too), `cachedSchema` read-only for the walk (O3: no IO on the walk, pinned by a spy test), `itemType` supplied by a declared schema document (§6.3), **heterogeneous-header superset normalization** (O19, the row-53 deferral), `JobUpstreamSchema` as an ordered provider list, `SortSpecEditor` dropdown, source-card **Columns** chrome. Columns are the whole design-time data surface — **no row preview**, deliberately (§9). **Absorbs J3b. M** | — (analysis) | `next/DS6` | ☐ |
| 57 | **DS7** — writers yield a `DataRef`; `ResultSinkWorker keep: all` | `result:` on the writers (blank = today's behaviour; set = yield `DataRef` / grouped `DataUnit` into a declared component via a second `ResultYielder` marker, capability-checked), `keep: all` (O2, memory stated), `ReadWorker` publishes its resolved manifest to the trace (§8.1 reproducibility), the §7.1 outer/inner worked shape as an engine fixture. The yielded ref is durable by construction (row 50's `DataSourceId`). Same file as J4's `groupBy` — sequence, don't interleave. **M** | — (analysis) | `next/DS7` | ☐ |
| 58 | **DS8** — `LogicDataSource` + the dated example | **Reframed 2026-08-21b** (D5, O16): rather than a ninth hand-written Kotlin source, the final session ships `LogicDataSource` — a source whose `units` delegates to a user-authored **Script / Flow / Job** through `DataScope.host`, minting plain refs that the shared flat-file opener reads — plus the dated-path case as a shipped **example Script**. Proves the extension point (a user can author a source, with tracing / stepping / breakpoints for free) instead of adding another class, and keeps the fused single-object shape available for JDBC where the coupling is necessary. `PathPatternSubstitution` shared with the writers' `${…}`; `missing: skip \| fail` from row 52. Kotlin `DatedPathDataSource` kept in reserve if date iteration in a Script proves clumsy. **Closes the arc — flip the analysis doc's status to landed. M** | — (analysis) | `next/DS8` | ☐ |
