...
| **E** | `2026-07-25_extensibility-improvements.md` | Plugin = classloader (process-global `KzenAutoRuntime`), plugin UX, Custom power, plain-object data shape, object-graph paths, closeable streams and items; absorbs reflection R5/R6. **E1 ratified 2026-09-04; reviews 1–5 of the hosting analysis folded in** | E2–E4, E7–E9 |
...
The **HS** constituent plan is [`in-process-hosting/README.md`](in-process-hosting/README.md):
25 sessions for the Java 25 baseline, native/persistent-graph sample, E2/E3/E7/E8/E9 execution and
Spring hosting. Its numbered files are permanent session records. Added 2026-09-04; all are open.
...
| HS01 | **HS01 — Java 25 baseline and local publication** | Prerequisites and acceptance in the session file | HS · HS01 | [session](in-process-hosting/01-java-25-baseline.md) | ☐ |
| HS02 | **HS02 — Plain-jar Spring compatibility gates** | Prerequisites and acceptance in the session file | HS · HS02 | [session](in-process-hosting/02-spring-compatibility-spike.md) | ☐ |
| HS03 | **HS03 — Plain Java core, ITCH reader and synthetic oracle** | Prerequisites and acceptance in the session file | HS · HS03 | [session](in-process-hosting/03-java-core-and-itch-reader.md) | ☐ |
| HS04 | **HS04 — Persistent locate-partitioned derived store** | Prerequisites and acceptance in the session file | HS · HS04 | [session](in-process-hosting/04-locate-partitioned-store.md) | ☐ |
| HS05 | **HS05 — Off-heap SymbolDay and persistent analytical graph** | Prerequisites and acceptance in the session file | HS · HS05 | [session](in-process-hosting/05-symbol-day-and-leak-detection.md) | ☐ |
| HS06 | **HS06 — Early real-day storage and memory gate** | Prerequisites and acceptance in the session file | HS · HS06 | [session](in-process-hosting/06-early-memory-measurement.md) | ☐ |
| HS07 | **HS07 — Process-global runtime and plugin scopes** | Prerequisites and acceptance in the session file | HS · HS07 | [session](in-process-hosting/07-runtime-and-plugin-scopes.md) | ☐ |
| HS08 | **HS08 — Provider descriptors and exact-origin notation** | Prerequisites and acceptance in the session file | HS · HS08 | [session](in-process-hosting/08-provider-and-notation-discovery.md) | ☐ |
| HS09 | **HS09 — Aggregate class loading, expressions and contextual availability** | Prerequisites and acceptance in the session file | HS · HS09 | [session](in-process-hosting/09-plugin-compilation-and-availability.md) | ☐ |
| HS10 | **HS10 — Context-owned work roots, host services and shutdown** | Prerequisites and acceptance in the session file | HS · HS10 | [session](in-process-hosting/10-context-roots-and-host-services.md) | ☐ |
| HS11 | **HS11 — Java-friendly reader and cursor-source adapters** | Prerequisites and acceptance in the session file | HS · HS11 | [session](in-process-hosting/11-java-blocking-adapters.md) | ☐ |
| HS12 | **HS12 — Plugin diagnostics and compatibility test kit** | Prerequisites and acceptance in the session file | HS · HS12 | [session](in-process-hosting/12-plugin-diagnostics-and-test-kit.md) | ☐ |
| HS13 | **HS13 — Plain-object shapes and expression inference** | Prerequisites and acceptance in the session file | HS · HS13 | [session](in-process-hosting/13-plain-object-shapes.md) | ☐ |
| HS14 | **HS14 — Recursive object contracts across JVM and JS** | Prerequisites and acceptance in the session file | HS · HS14 | [session](in-process-hosting/14-recursive-contracts.md) | ☐ |
| HS15 | **HS15 — E9 ownership ledger and native lifetime primitives** | Prerequisites and acceptance in the session file | HS · HS15 | [session](in-process-hosting/15-run-ownership-primitives.md) | ☐ |
| HS16 | **HS16 — E9 leases through channels and Worker callbacks** | Prerequisites and acceptance in the session file | HS · HS16 | [session](in-process-hosting/16-channel-and-worker-ownership.md) | ☐ |
| HS17 | **HS17 — E9 source acquisition, streams and live-edit migration** | Prerequisites and acceptance in the session file | HS · HS17 | [session](in-process-hosting/17-source-acquisition-and-migration.md) | ☐ |
| HS18 | **HS18 — E9 result snapshots, diagnostics and acceptance** | Prerequisites and acceptance in the session file | HS · HS18 | [session](in-process-hosting/18-result-boundaries-and-ownership-gate.md) | ☐ |
| HS19 | **HS19 — Object-graph path projection and unnesting** | Prerequisites and acceptance in the session file | HS · HS19 | [session](in-process-hosting/19-object-graph-projection.md) | ☐ |
| HS20 | **HS20 — Design-time object-graph path picker** | Prerequisites and acceptance in the session file | HS · HS20 | [session](in-process-hosting/20-object-graph-path-picker.md) | ☐ |
| HS21 | **HS21 — ITCH and cities Job reader adapters** | Prerequisites and acceptance in the session file | HS · HS21 | [session](in-process-hosting/21-sample-reader-adapters.md) | ☐ |
| HS22 | **HS22 — Analytical Workers, bundled Jobs and standalone plugin proof** | Prerequisites and acceptance in the session file | HS · HS22 | [session](in-process-hosting/22-analytical-jobs-and-standalone-proof.md) | ☐ |
| HS23 | **HS23 — Spring host workspaces, lifecycle and streaming proxy** | Prerequisites and acceptance in the session file | HS · HS23 | [session](in-process-hosting/23-spring-host-and-proxy.md) | ☐ |
| HS24 | **HS24 — Live host objects and shared memory governance** | Prerequisites and acceptance in the session file | HS · HS24 | [session](in-process-hosting/24-host-objects-and-memory-governor.md) | ☐ |
| HS25 | **HS25 — Integrated acceptance, real-day pressure and documentation** | Prerequisites and acceptance in the session file | HS · HS25 | [session](in-process-hosting/25-integrated-acceptance-and-docs.md) | ☐ |
| 10 | **E2 — decomposed into sessions** | Execution is HS07–HS11 + HS22; the E plan retains design authority and its phase stays open until all acceptance passes. This row is superseded as a session, not implemented. | E · E2 | [HS tracker](in-process-hosting/README.md) | ⊘ |
| 11 | **E3 — decomposed into sessions** | Execution is HS12 + HS21–HS22; the E plan retains design authority and its phase stays open until all acceptance passes. This row is superseded as a session, not implemented. | E · E3 | [HS tracker](in-process-hosting/README.md) | ⊘ |
...
| 59 | **E7 — decomposed into sessions** | Execution is HS13–HS14; the E plan retains design authority and its phase stays open until all acceptance passes. This row is superseded as a session, not implemented. | E · E7 | [HS tracker](in-process-hosting/README.md) | ⊘ |
| 60 | **E8 — decomposed into sessions** | Execution is HS19–HS20; the E plan retains design authority and its phase stays open until all acceptance passes. This row is superseded as a session, not implemented. | E · E8 | [HS tracker](in-process-hosting/README.md) | ⊘ |
| 61 | **E9 — decomposed into sessions** | Execution is HS15–HS18; the E plan retains design authority and its phase stays open until all acceptance passes. This row is superseded as a session, not implemented. | E · E9 | [HS tracker](in-process-hosting/README.md) | ⊘ |
...
The in-process hosting arc is planned as **HS01–HS25** in
[`in-process-hosting/`](in-process-hosting/README.md), with one file per session. It includes the
E2/E3/E7/E8/E9 execution slices; their old single-session rows are superseded, not implemented.
HS01 is the entry point when this arc is selected. G1–G10 and P1 are gates within the plan, not
assumed results. HS06 measures off-heap messages and the persistent heap graph before integration.
...
ratified on 2026-09-04; **HS session prerequisites replace the one-session E2/E7/E9 scheduling**.
Their logical independence does not imply disjoint files. E9 closes at HS18; the plugin's external
E2/E3 acceptance closes at HS22.
...
5. **E1 ratified 2026-09-04; E2/E3/E7/E8/E9 execute through HS01–HS25's session prerequisites.**
   Serialize their actual shared-file edits with each other and J5b/J4/J9/J7; no overlapping Job
   ownership/migration work runs concurrently. E4's **C7 half is independent of all** — the document-rename gap is a
