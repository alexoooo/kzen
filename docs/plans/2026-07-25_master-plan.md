...
| 10 | **E2** — plugin = classloader | One **process-global `KzenAutoRuntime`** (initialized once before any context; conflicting re-init fails fast) owning one `URLClassLoader` per `plugins/<name>/` jar set (`--plugin.root=`, deterministic order, duplicate ids / readers / notation paths are boot errors, per-scope failure isolation), app loader as plugin zero; implicit plugin id = canonical directory name with a reserved `application` id (review 4); per loader: `ServiceLoader` readers de-duplicated by declaring loader and **exact-origin** bundled notation (scanned over the scope's own jar URLs and read through the exact resource URL — `ClassPath.from` walks ancestors and `getResource` is parent-first; review 4); **one `ReflectiveClassMirror` over the runtime's aggregate delegating loader** (lazy — no boot validation promise; a `@Reflect` name in two folder scopes is a resolution-time ambiguity, review 2; **parent-first, the application classpath wins**, a shadowed folder class is a named warning, review 3); `dynamicParentClassLoader()` returns that aggregate (zero call-site changes for loading) and `ScriptKotlinCompiler` receives the explicit plugin-jar union (review 2: `classpathFromClassloader` cannot see through a delegating loader); plugin `@Service` needs validated **per context** in a per-context availability view initialized at creation and monotonically augmented by lazy reflective resolution, surfaced as "unavailable in this workspace", never blocking and never mutating global state (reviews 2, 3, 5); the runtime holds `ServiceLoader` provider descriptors and each context instantiates its own capabilities (review 3); the runtime claims live work roots (create → `toRealPath()` → claim; released after server stop and run join, in that order, with construction rollback — review 5), a duplicate fails fast (reviews 2, 3); `BlockingReaderCapability` + **cursor-driven** `SourceWorker` in `kzen-auto-plugin`; `Class<?>`-keyed `KzenAutoHost` builder for Java hosts; optional manifest with SPI-compatibility version; `plugins.yaml` / plugin `ReportDefiner` discovery retired. Verified pure-Java from a folder (sample plugin, unshaded) **and** as a dependency **and** Kotlin+KSP, plus the two-plugin expression / identity / ambiguity gate, the exact-origin notation gate and a reusable plugin compatibility test kit; boot-error cases run in a forked `pluginUniverseTest` task against their own universe, the rest against one constructed test universe (review 4). **L; needs the HS Java 25 baseline first** | E · E2 | — | ☐ |
...
| 60 | **E8** — object-graph paths | Path-projection / unnest Worker (`instrument.symbol`, `executions[*].price` → flat columns, one row per element; null intermediate → null leaf, empty list → zero rows, same-list `[*]` share the element; output name = full dotted path with wildcards dropped, explicit aliases, duplicates rejected, scalar leaves, maps unnest to `key` / `value` — reviews 2, 3) + design-time path picker over the upstream contract. The one new Worker for reporting over an object graph without code. **M** | E · E8 | — | ☐ |
| 61 | **E9** — closeable streams and items | **Added 2026-09-04 (user requirement via review 1 of the hosting analysis).** `AutoCloseable` honoured on every expression stream type (`Iterable` / `Iterator` / `Sequence` + new `java.util.stream.Stream`) — closed on completion / cancel / failure, detached and re-adopted across live-edit migration instead of re-evaluated; and on **individual elements** — emitting an `AutoCloseable` transfers ownership to the run, never at `lift` (review 3) — **at source ingress for a pulled item** (producer lease from `next()` through lift / projection / send; a lift failure or a projection to scalars closes it) and **at the run-scoped send for a Worker-created one** (review 4; a borrowed host object is wrapped) — ingress only where the framework owns the pull, a Kotlin `produce` or expression body owning its pre-emit interval (review 5); an owned native **never escapes the run** — Result / trace / preview boundaries take a run-scoped structural snapshot via `JobControl.snapshot` or fail by name (review 5); a closeable child a Formula returns is `Borrowed`, a new one transfers (review 5); ownership is **linear across runs**, enforced fail-fast by one process-level weak `NativeIdentityRegistry` (owned-by-run / closed) that also serves as the post-close tombstone (review 4); a per-run ledger keyed by native identity counts **leases with named holders** (review 2): a channel from `send` until the consumer's loop hands the element to `onElement`, the Worker implicitly for the duration of the callback (the framework loops consume batches via `receiveBatch()`, so this is where the release lives, per element), and an accumulator **explicitly** via `control.retain` — the only Worker cooperation, and only for retention; last release closes exactly once and marks the identity closed in that registry (re-adoption is the named use-after-close error, review 3); owner propagation (an **owner set**) to navigation children and to non-scalar Formula / Filter outputs; flush-on-send for owned elements; carried unclosed through `drainBuffered` / `preload`; lease holder = Worker location so migration re-adoption is a no-op; force-close in `JobRun`'s `finally` after the existing `coroutineScope` join, a throwing `close()` does not stop the rest and never displaces a primary processing failure (review 5); post-close access a named error; and proportional diagnostics — counts by holder in progress, per-item detail on demand, a stall *warning* naming the accumulating Worker after the deadlock monitor's no-progress interval (review 3; no validation rule — inherent complexity, surfaced not prevented). This is the mechanism behind the in-process host's memory arena (a symbol-day batch consumes the arena on open, releases on close); kzen needs no admission concept. **M–L; independent of E2 and E7** | E · E9 | — | ☐ |
...
`kzen-sample-plugin` over real market data) is **analysed and reviewed five times** — `docs/analysis/2026-09-03_in-process-hosting.md`;
its questions are all decided and only its §9 gates remain, but nothing from it is scheduled on this ledger yet — the E rows carry the kzen-side work.
