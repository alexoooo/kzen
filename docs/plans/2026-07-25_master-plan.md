...
| 2 | **J3b** — design-time services | ~~Column pre-scan route + `JobUpstreamSchema` fallback on `WorkerLane` + `SortSpecEditor` dropdown~~ **SUPERSEDED 2026-08-21 by DS6** (row 56): the pre-scan is `DataSourceActions action=shape` over the opener's `inspectShape`, the fallback is a provider list on `JobUpstreamSchema` | J · P3 steps 2,3 | `next/J3` | ⊘ |
...
| 49 | **DS0** — schema vocabulary out of Report | **DS arc prep, added 2026-08-21b by the second-pass review** (`docs/analysis/2026-08-20_job-data-source.md` §10, §13). Mechanical, import-only: `HeaderListing` / `HeaderLabel` / `HeaderLabelMap` move from `common/objects/document/report/listing/` to `common/data/schema/`. ~70 files, zero behaviour. **Must precede row 50** or the new model types cement the wrong import; Job already reaches into Report for ~25 types and this is the subset the DS arc would otherwise deepen. **S** | — (analysis) | `next/DS0` | ☐ |
| 50 | **DS1** — data-source value model + lowering | **Start of the DS arc (Job data sources), added 2026-08-21 on direct user request; rationale lives in `docs/analysis/2026-08-20_job-data-source.md` (+ `2026-08-21_extension-points.md`), which is the permanent record — the `next/DS*` files are deleted on landing and as-built notes go to the analysis doc §14. Third-pass review 2026-08-23 recorded in its §15 (the non-suspend `DataScope` could not delegate to suspend `JobControl.runBlockingIo` / `host`; the recut is suspend SPI + worker-owned reads + plain file refs).** `DataRole` / `DataRef` / `DataPart` / `DataUnit` / `DataManifest` / `DataSourceId` / `DataResolveResult` / `DataShape` in kzen-auto-common with digest, `ExecutionValue` lowering, kotlinx wire form, `DataLocation` ⇄ `DataRef`, and the construction helpers a user-authored source needs. **`DataRef.source` is a nullable `DataSourceId` that nothing mints in this arc** — every v1 ref is a plain path (§15 D8, O15 deferred to the first provider-bound source); attribute order presentation-only, digest sorts keys (C13); reserved fingerprint keys `size` / `modified` (D9). **S–M, common only, no consumers yet** | — (analysis) | `next/DS1` | ☐ |
| 51 | **DS1b** — expression type-visibility fix | **A standing bug the DS arc would have tripped over, found 2026-08-21b** (§5.5a, C2): `ExpressionReturnTypeInference.toTypeMetadata` erases any classifier outside an eleven-entry hardcoded `visibleBuiltins` and the one-entry `ObjectRegistry` down to `Any` — so a `DataUnit` on a lane would type as `Any`. Replace the whitelist with a **public-and-nameable predicate** (no hardcoding, no `ObjectRegistry` round-trip for first-party code; the registry widens only predicate false-negatives, never `internal` types), and `isIterable` with **`isStreamType`** covering `Iterable \| Sequence \| Iterator` (O6). Independent of the arc and worth landing regardless; still a prerequisite for the typed unit lane (rows 53, 55). **S; may ride inside row 50** | — (analysis) | `next/DS1b` | ☐ |
| 52 | **DS2** — the suspend runtime: `DataSource` / `DataOpener` / `DataCursor` / `DataContext`, `FileDataSource` + `FileDataOpener`, `DataSourceActions` | **Rewritten 2026-08-23** (§15 C10, C11, D6, D8, D10, D11). **`resolve` / `open` are `suspend`** and take a `DataContext` (suspend `blocking` / `host` — the only shape that can reach `JobControl.runBlockingIo` / `host`); **`DataSource` = resolve only, `DataOpener` = describe + open**; **`DataCursor: Iterator`** is a plain pull reader the worker drives inside `runBlockingIo` (the `CsvRecordReader` precedent, O20); `FileDataSource` resolves to **plain refs** with a stamped fingerprint (one unit per file, role `main`, named-capture group pattern → attributes, `missing: skip \| fail` → diagnostics) and the shared **`FileDataOpener`** over `FileFlatDataSource` / `ReportDefiner` reads them (**J3a's reader re-homed into the opener**); `DataOpenerLookup` (plain → file opener; sourced → clear failure); **no id minting, no resolver, no `DataSources` document**; one generic `DataSourceActions` detached object for design-time resolve (`DataResolveResult`); the Job `sources:` branch + graph-wide discovery; the input plumbing moved out of Report (§10). **Spike O14 first — blank, set, cross-document, and delete (C15).** **M–L** | — (analysis) | `next/DS2` | ☐ |
| 53 | **DS3** — `ReadWorker` (source-generic declarative reader) | **Rewritten 2026-08-23.** `source:` a **nullable structural reference** (`is: DataSource, nullable: true`) so the instance comes from the run's own graph (D2); `emit: items \| units` (O11), `role:`, **`attributes: ignore \| columns`** — unit attributes as leading columns, the `groupBy` parity path (D12, O22); parts open via `DataOpenerLookup`; **every cursor call inside `runBlockingIo`** (C11, pinned by an offload counter in the A/B); resolved manifest **carried** across migration with the open cursor detached and driven by the new control; `payloadFlow` from `staticShape(role)` (C12). Heterogeneous item schemas **fail loudly** (C3, §5.2b); superset in row 56. Lifts the drain core (`DataReadCore`) for row 55. Removes `CsvReaderWorker` / `MultiFileReaderWorker` from the ribbon (archetypes stay). A/B vs both old readers. **M–L** | — (analysis) | `next/DS3` | ☐ |
| 54 | **DS4** — sources editing surface | Job "Data sources" section + `Data` ribbon group (insert nests under `sources` **by capability**, `drop(1)` so the abstract archetype does not offer itself), `DataSourceCard` rendering attributes through `AttributeEditorManager` with generic **Resolve** chrome through `DataSourceActions` (rendering `DataResolveResult` + diagnostics — D10), `FileSelectionEditor` (= `MultiFileInputEditor` rewritten as `FileDataSource.files`' editor, with the `filter` field labelled for what it is — a contains-all-words match, **not** a glob, C6), `SelectDataSourceEditor` (graph-wide) with **auto-bind when exactly one source** plus an in-card bound-source summary. **No id minting** (O15 deferred); smoke includes **delete-the-bound-source** (C15). **Zero file-specific code in the Job editor.** ⚠ An `editor:` key and its `@Reflect` registration must land together (C5). **L, split 1–3 / 4–6; browser smoke is the deliverable — with row 53 this makes "count the lines of one file" three cards and no code** | — (analysis) | `next/DS4` | ☐ |
| 55 | **DS5** — `ReadPartWorker` + the 1:N transform cadence | **Rewritten 2026-08-23** (was "`ExpandWorker` + objects in expression scope"; §15 D7 — expressions never initiate source I/O, so objects-in-scope / `items()` helpers / the run-time resolver fallback are gone and `ExpandWorker` is demand-driven, O1). The **second data reader**: a `TransformWorker` that opens `unit.part(role)` off a `DataUnit` lane through row 53's `DataReadCore`, `attributes: columns`, cursor scoped to one unit, migration carries `(inputElementIndex, itemIndex, cursor)`; plus **`Emitter.expandCadence`** (flush + checkpoint every N sends *within* `onElement` — §5.4b, the one expression-route cost that survives) and handle ownership. The child-Logic idiom is `FormulaSourceWorker("unit")` → `ReadPartWorker`, proven in an engine fixture across the Logic boundary with no resolver. Needs rows 51, 53. **M** | — (analysis) | `next/DS5` | ☐ |
| 56 | **DS6** — design-time shape | **Rewritten 2026-08-23** (§15 C12, D9). `DataShape` (`Tabular` \| `Object`); **`DataOpener.staticShape(role)`** notation-only for the walk (no `part` exists at walk time), **`suspend inspectShape(context, part)`** bounded for the editor via `DataSourceActions action=shape`; a **`SchemaCache` service** keyed on the fingerprint row 52 stamps into the ref (fixes Report's stale-`columns.csv` bug too), `peek` read-only for the walk (O3, pinned by a spy test) — **not** an SPI member; the declared-schema route over the shipped-but-orphaned `DataFormat` document (§6.3, O18 — user-facing rename); **superset normalization** behind `mergeSchemas` on both readers (O19, the row-53 deferral); `JobUpstreamSchema` provider list, `SortSpecEditor` dropdown, source-card **Columns** chrome. **No row preview** (§9). **Absorbs J3b. M–L** | — (analysis) | `next/DS6` | ☐ |
| 57 | **DS7** — writers yield a `DataRef`; `ResultSinkWorker keep: all` | `result:` on the writers (blank = today's behaviour; set = yield `DataRef` / grouped `DataUnit` into a declared component via a second `ResultYielder` marker, capability-checked), `keep: all` (O2, memory stated), `ReadWorker` publishes its resolved manifest to the trace (§8.1 reproducibility, fingerprints included), the §7.1 outer/inner worked shape as an engine fixture (child reads with `ReadPartWorker`). **Same-run composition only** — the yielded ref is a plain path, durable by construction; no resolver, no rename-restart regression (§15 D8); a persisted cross-run results registry is explicitly out (§7.1). ⚠ `ExecutionValue.ofArbitrary` does not lower a `DataRef`. Same file as J4's `groupBy` — sequence, don't interleave. **M** | — (analysis) | `next/DS7` | ☐ |
| 58 | **DS8** — `LogicDataSource` + named host arguments + the dated example | **Reframed 2026-08-21b** (D5, O16), **revised 2026-08-23** (§15 C14, D7, D8): the final session ships `LogicDataSource` — a **resolve-only** source whose `resolve` delegates to a user-authored **Script / Flow / Job** through `DataContext.host` with **named arguments** (a `JobControl.host(instructions, arguments: TupleValue)` overload; the 2026-08-21b one-map-positional form did not match its own two-parameter example), minting plain refs the shared `FileDataOpener` reads — plus the dated-path case as a shipped **example Script**. Proves the extension point (a user can author a source, with tracing / stepping / breakpoints for free) instead of adding another class, and keeps the fused shape available for JDBC where the coupling is necessary. `PathPatternSubstitution` shared with the writers' `${…}`. Carries the arc's acceptance fixture (outer/inner ETL round-trip, no expression I/O). Kotlin `DatedPathDataSource` kept in reserve if date iteration in a Script proves clumsy. **Closes the arc — flip the analysis doc's status to landed. M** | — (analysis) | `next/DS8` | ☐ |
