...
| 49 | **DS0** — schema vocabulary out of Report | **DS arc prep** (`docs/analysis/2026-08-20_job-data-source.md` §10). Mechanical, import-only: `HeaderListing` / `HeaderLabel` / `HeaderLabelMap` move from `common/objects/document/report/listing/` to `common/data/schema/`. ~70 files, zero behaviour. **Must precede row 50** or the new model types cement the wrong import; Job already reaches into Report for ~25 types and this is the subset the DS arc would otherwise deepen. **S** | — (analysis) | `next/DS0` | ☐ |
| 50 | **DS1** — data-source value model + lowering | **Start of the DS arc (Job data sources), added 2026-08-21 on direct user request; rationale lives in `docs/analysis/2026-08-20_job-data-source.md` (+ `2026-08-21_extension-points.md`), which is the permanent record — the `next/DS*` files are deleted on landing and as-built notes go to the analysis doc §13.** `DataRole` / `DataRef` / `DataPart` / `DataUnit` / `DataManifest` / `DataSourceId` / `DataResolveResult` / `DataShape` in kzen-auto-common with digest, `ExecutionValue` lowering, kotlinx wire form, `DataLocation` ⇄ `DataRef`, and the construction helpers a user-authored source needs. **`DataRef.source` is a nullable `DataSourceId` that nothing mints in this arc** — every v1 ref is a plain path (O15 — deferred to the first provider-bound source); attribute order presentation-only, digest sorts keys; reserved fingerprint keys `size` / `modified` (§6.2). **S–M, common only, no consumers yet** | — (analysis) | `next/DS1` | ☐ |
| 51 | **DS1b** — expression type-visibility fix | **A standing bug the DS arc would trip over** (§5.5): `ExpressionReturnTypeInference.toTypeMetadata` erases any classifier outside an eleven-entry hardcoded `visibleBuiltins` and the one-entry `ObjectRegistry` down to `Any` — so a `DataUnit` on a lane would type as `Any`. Replace the whitelist with a **public-and-nameable predicate** (no hardcoding, no `ObjectRegistry` round-trip for first-party code; the registry widens only predicate false-negatives, never `internal` types), and `isIterable` with **`isStreamType`** covering `Iterable \| Sequence \| Iterator` (O6). Independent of the arc and worth landing regardless; still a prerequisite for the typed unit lane (rows 53, 55). **S; may ride inside row 50** | — (analysis) | `next/DS1b` | ☐ |
| 52 | **DS2** — the suspend runtime: `DataSource` / `DataOpener` / `DataCursor` / `DataContext`, `FileDataSource` + `FileDataOpener`, `DataSourceActions` | **`resolve` / `open` are `suspend`** and take a `DataContext` (suspend `blocking` / `host` — the only shape that can reach `JobControl.runBlockingIo` / `host`); **`DataSource` = resolve only, `DataOpener` = describe + open**; **`DataCursor: Iterator`** is a plain pull reader the worker drives inside `runBlockingIo` (the `CsvRecordReader` precedent, O20); `FileDataSource` resolves to **plain refs** with a stamped fingerprint (one unit per file, role `main`, named-capture group pattern → attributes, `missing: skip \| fail` → diagnostics) and the shared **`FileDataOpener`** over `FileFlatDataSource` / `ReportDefiner` reads them (**J3a's reader re-homed into the opener**); `DataOpenerLookup` (plain → file opener; sourced → clear failure); **no id minting, no resolver, no `DataSources` document**; one generic `DataSourceActions` detached object for design-time resolve (`DataResolveResult`); the Job `sources:` branch + graph-wide discovery; the input plumbing moved out of Report (§10). **Spike O14 first — blank, set, cross-document, and delete.** **M–L** | — (analysis) | `next/DS2` | ☐ |
| 53 | **DS3** — `ReadWorker` (source-generic declarative reader) | `source:` a **nullable structural reference** (`is: DataSource, nullable: true`) so the instance comes from the run's own graph; `emit: items \| units` (O11), `role:`, **`attributes: ignore \| columns`** — unit attributes as leading columns, the `groupBy` parity path (O22); parts open via `DataOpenerLookup`; **every cursor call inside `runBlockingIo`** (pinned by an offload counter in the A/B); resolved manifest **carried** across migration with the open cursor detached and driven by the new control; `payloadFlow` from `staticShape(role)`. Heterogeneous item schemas **fail loudly** (§5.3); superset in row 56. Lifts the drain core (`DataReadCore`) for row 55. Removes `CsvReaderWorker` / `MultiFileReaderWorker` from the ribbon (archetypes stay). A/B vs both old readers. **M–L** | — (analysis) | `next/DS3` | ☐ |
| 54 | **DS4** — sources editing surface | Job "Data sources" section + `Data` ribbon group (insert nests under `sources` **by capability**, `drop(1)` so the abstract archetype does not offer itself), `DataSourceCard` rendering attributes through `AttributeEditorManager` with generic **Resolve** chrome through `DataSourceActions` (rendering `DataResolveResult` + diagnostics), `FileSelectionEditor` (= `MultiFileInputEditor` rewritten as `FileDataSource.files`' editor, with the `filter` field labelled for what it is — a contains-all-words match, **not** a glob), `SelectDataSourceEditor` (graph-wide) with **auto-bind when exactly one source** plus an in-card bound-source summary. **No id minting** (O15 deferred); smoke includes **delete-the-bound-source**. **Zero file-specific code in the Job editor.** ⚠ An `editor:` key and its `@Reflect` registration must land together. **L, split 1–3 / 4–6; browser smoke is the deliverable — with row 53 this makes "count the lines of one file" three cards and no code** | — (analysis) | `next/DS4` | ☐ |
| 55 | **DS5** — `ReadPartWorker` + the 1:N transform cadence | Expressions never initiate source I/O, so a generic in-memory `ExpandWorker` is demand-driven (O1). The **second data reader**: a `TransformWorker` that opens `unit.part(role)` off a `DataUnit` lane through row 53's `DataReadCore`, `attributes: columns`, cursor scoped to one unit, migration carries `(inputElementIndex, itemIndex, cursor)`; plus **`Emitter.expandCadence`** (flush + checkpoint every N sends *within* `onElement` — §5.4, the cadence both 1:N workers need — survives) and handle ownership. The child-Logic idiom is `FormulaSourceWorker("unit")` → `ReadPartWorker`, proven in an engine fixture across the Logic boundary with no resolver. Needs rows 51, 53. **M** | — (analysis) | `next/DS5` | ☐ |
