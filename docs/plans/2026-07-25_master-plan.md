...
| 54 | **DS4** — source-worker editing surface | **Corrected after UI review:** no detached `Data sources` section, `Data` ribbon, or `DataSourceCard`. **File** / **Logic** are ordinary source Workers in the existing Sources ribbon: select the tool, choose a normal stage insertion gap, then edit the ordinary `WorkerDisplayDefault` card. They reuse row 53's manifest/projection/cursor/migration engine over channel-free `FileDataSource` / `LogicDataSource` delegates and participate in the same ordering, drag/drop, channel synthesis, validation and progress paths as every Worker. Pure DataSource objects + nominal `ReadWorker` remain available for authored cross-document use; `FileSelectionEditor`, graph-wide `SelectDataSourceEditor`, auto-bind, and source actions remain for that model. Inline Workers intentionally omit detached Resolve/Columns chrome; declared schema still reaches server validation and live Summary supplies downstream editor suggestions. **No id minting; no file/logic branch in `JobController`.** | — (analysis) | `next/DS4` | ☑ 2026-08-24 |
